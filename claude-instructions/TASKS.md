...
- [ ] **[S1-D1-09]** `widgets/top-bar`: тонкая полоска над шапкой (требование оригинального ТЗ). Телефоны (click-to-call `tel:`), email, адрес, иконки соцсетей (VK, WhatsApp, Telegram, Avito). Данные из SiteSettings (ISR). SVG-иконки как компоненты.

- [ ] **[S1-D1-10]** `widgets/header`: логотип + навигация. Dropdown для "Дополнительные услуги" (список из пропсов). Sticky при скролле (добавляет класс с bg и shadow). Бургер-меню → drawer на мобильных. Dropdown внутри drawer — аккордеон (не hover). Пункты: Аренда офисов, Доп. услуги, Фотогалерея, Расположение, О нас → /about, Контакты.

- [ ] **[S1-D1-11]** `widgets/footer`: контакты, ссылки на разделы, соцсети, копирайт, ссылка на `/privacy` (Политика конфиденциальности).

- [ ] **[S1-D1-12]** `widgets/floating-cta`: fixed кнопка "Напишите нам". На мобильных — круглый FAB. Клик → открывает Modal с LeadForm. Присутствует в `app/(public)/layout.tsx`.
...
- [ ] **[S2-D1-10]** `src/app/(public)/about/page.tsx`: страница "О бизнес-центре" (требование оригинального ТЗ, раздел 5 структуры сайта). Описание компании, история, инфраструктура, преимущества аренды. Данные из SiteSettings или статичный контент. ISR revalidate: 600. `generateMetadata`.
...
- [ ] **[S3-D2-09]** Брутфорс-защита входа в админку (требование dev-plan-v2, раздел 6.1). В `src/auth.ts` добавить счётчик неверных попыток по IP: после 5 неверных попыток — блокировка на 15 мин (in-memory Map с TTL). Сбрасывается при успешном входе.
...
- [ ] **[S4-D2-07]** `src/app/(public)/privacy/page.tsx`: Политика конфиденциальности (требование оригинального ТЗ, обязательная ссылка из footer). Статичный текст или из SiteSettings. SEO: noindex. `generateMetadata`.

---

## Backlog (на будущее / по запросу клиента)

<!-- /about и /privacy перенесены в спринты S2-D1-10 и S4-D2-07 —
     требования оригинального ТЗ, не опциональный функционал -->
